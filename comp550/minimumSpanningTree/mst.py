class Solution:
    ...
    def prims(self, graph):
        ...
        for _ in range(totalVertices - 1): # MST will always have 1 less than vertex's
            bestEdge = None

            for currentVertex in range(totalVertices):
                if inTree[currentVertex]: 
                    for neighbor, weight in graph[currentVertex]: 
                        if not inTree[neighbor]:
                            if bestEdge is None or weight < bestEdge[2]:
                                bestEdge = (currentVertex, neighbor, weight)
